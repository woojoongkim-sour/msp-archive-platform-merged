...
async def document_progress(document_id: UUID, db: Session = Depends(get_db)):
    """문서 처리 진행 상황 SSE 스트림 (Redis pub/sub → DB 상태 폴링 fallback)."""
    import asyncio

    doc_id_str = str(document_id)

    async def _redis_stream() -> AsyncIterator[str]:
        try:
            import redis.asyncio as aioredis
            redis_url = os.getenv("REDIS_URL", "redis://redis:6379/0")
            r = aioredis.from_url(redis_url)
            channel = f"doc_progress:{doc_id_str}"
            pubsub = r.pubsub()
            await pubsub.subscribe(channel)
            try:
                timeout = 120  # max 2분 대기
                elapsed = 0
            ...
            # Redis 불가 → DB 상태 폴링
            async for event in _db_poll_stream():
                yield event

    async def _db_poll_stream() -> AsyncIterator[str]:
        from app.db.session import SessionLocal
        for _ in range(60):
            await asyncio.sleep(2)
            poll_db = SessionLocal()
            try:
                doc = poll_db.query(Document).filter(Document.id == document_id).first()
                if not doc:
                    yield f"data: {json.dumps({'step': 'error', 'status': 'not_found'})}\n\n"
                    return
                status = doc.processing_status
                yield f"data: {json.dumps({'step': 'processing', 'status': status})}\n\n"
                if status in ("completed", "failed", "blocked"):
                    yield f"data: {json.dumps({'step': 'complete', 'status': status})}\n\n"
                    return
            finally:
                poll_db.close()
